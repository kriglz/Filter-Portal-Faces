...
class Renderer {
    ...
    init(session: ARSession, metalDevice device: MTLDevice, renderDestination: RenderDestinationProvider) {
        ...
        self.scnRenderer = SCNRenderer(device: device, options: nil)
        loadScene()
        loadMetal()
        loadAssets()
    }
    ...
    func loadScene() {
        timer = Timer.scheduledTimer(withTimeInterval: 0.033333, repeats: true, block: { [weak self] _ in
            self?.renderTime += 0.03333
        })
        
        scnRenderer.autoenablesDefaultLighting = true
        scnRenderer.isPlaying = true
        scnRenderer.pointOfView = cameraNode
        
        let scene = SCNScene(named: "TheScene.scn")!
        scene.background.contents = UIColor.red
        scene.isPaused = false
        scnRenderer.scene = scene
        
        scnRenderer.scene?.rootNode.addChildNode(faceNode)
    }
    ...
    func updateGameState() {
        ...
        updateFaceAnchor(frame: currentFrame)
    ...
    func updateFaceAnchor(frame: ARFrame) {
        ...
        let arCameraFieldOfView = scnRenderer.pointOfView?.camera?.projectionDirection == SCNCameraProjectionDirection.vertical
            ? frame.camera.fieldOfView.width.rounded()
            : frame.camera.fieldOfView.height.rounded()
        scnRenderer.pointOfView?.camera?.fieldOfView = arCameraFieldOfView
    ...
    func drawScene(commandBuffer: MTLCommandBuffer, passDescriptor: MTLRenderPassDescriptor) {
